...
1.*Resolve image path:** `$0`. `` to confirm it's an image (png/jpg/webp/heic).
...
After ingest, always offer to pair with a real source (Figma, codebase) to lock in exact values — the screenshot gives astarting direction*, not ground truth.
