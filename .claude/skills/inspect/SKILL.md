...
- Chrome DevTools (via mcporter) has a page loaded
- `.claude/last-snapshot.json` exists — or we snapshot fresh

## Pipeline

1.*Get snapshot:**
 - Prefer `read .claude/last-snapshot.json` if recent (< 5 min, same URL)
 - Otherwise take fresh snapshot via mcporter: `mcporter(action="call", selector="mcporter ")`
...
4.*Resolve to source location.** Once UID is confirmed:
 - `` via mcporter — select by UID and return useful identifiers:
 ```js
 ...
 ```
 - With these signals, use `bash` (`grep`) on the source HTML (current file) for the matching element — preferring `id` > `data-*` > unique class combination > text snippet > outerHTML substring.
...
- Elements inside shadow DOM are visible to `` but `grep` on source HTML won't find them — warn the user if this path fails
