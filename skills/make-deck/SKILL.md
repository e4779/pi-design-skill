...
2. `` — org-level registry. If the brief mentions a brand name matching a folder → auto-apply it (e.g. brief "Acme deck" + `~/.claude/design-systems/acme/` → use Acme)
3. `bash*/tailwind.config.* **/theme.{ts,js,json} **/tokens.{css,scss} **/_variables.*` at project root — if found, note "codebase detected" and `Read` them
...
1. Create `artifacts/<slug>.html` with shell:
 ...
 ```
2. `/")` — copy starter into the*same dir as the HTML** (important for nested paths like `artifacts/foo/deck.html`)
...
3.*Mandatory programmatic overflow audit** (blocks end-of-turn if any slide overflows or is too tight):
 ...
 >*If the browser shows an overflow that this script says doesn't exist**, hard-reload with cache-bust: `mcporter ({ type: "reload", ignoreCache: true })`. Most such disagreements come from a cached CSS state prior to your latest `Edit`.
